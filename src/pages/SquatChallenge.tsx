...
if (sessionStartTime && squatTimes.length > 0) {
  ...
  // 세션 데이터를 블록체인에 기록하는 함수 호출 (별도 구현 필요)
  // recordSessionToBlockchain(sessionSquats, sessionStartTime, Date.now(), squatTimes);
}
...
return (
<Container> <h1>Squat Challenge</h1> {!isFullBody && ( <Instructions> 전신이 카메라에 보이도록 뒤로 물러서세요. 측면으로 서면 스쿼트 감지가 더 정확합니다. </Instructions> )} <div> 오늘의 목표: {todayCount} / {dailyGoal} 스쿼트 <ProgressBar> <Progress width={progressPercentage} /> </ProgressBar> </div> <div style={{ marginBottom: '20px', padding: '10px', backgroundColor: sessionActive ? '#e6ffe6' : '#ffe6e6', borderRadius: '5px' }}> {!sessionActive ? ( <> <p>세션이 비활성화되어 있습니다. 세션을 시작하면 스쿼트 기록이 더 정확해집니다.</p> <ModalButton onClick={startSession}>세션 시작</ModalButton> </> ) : ( <> <p>세션 활성화됨: {sessionSquats} / {MAX_SESSION_SQUATS} 스쿼트</p> <ModalButton onClick={endSession}>세션 종료</ModalButton> </> )} </div> <VideoContainer> <Video ref={videoRef} playsInline /> <Canvas ref={canvasRef} /> </VideoContainer> <Counter>{count}</Counter> <div>Total Squats: {totalSquats}</div>
{showCelebration && (
<CelebrationModal>
<ModalContent>
